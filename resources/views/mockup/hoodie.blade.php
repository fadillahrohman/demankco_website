    ...
    <div class="h-50 mb-2 flex w-fit grid-cols-2 gap-x-1 justify-self-center rounded-lg bg-white p-3 shadow-sm outline outline-1 outline-slate-200"
        ...
        <div id="kiri"
            ...
            <button
                ...
                <a href="{{ route('orderHoodie') }}" onclick="saveMockupImage()">Pesan</a>
            </button>
            <button
...
@push('fabric_scripts')
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            fabric.Image.fromURL("{{ asset('images/mockup-hoodie.png') }}", function(img) {
                ...
                <label class="block text-sm font-medium">Warna Hoodie:</label>
                <div class="grid grid-cols-3 gap-2">
                    ...
                    <button class="w-full h-8 rounded bg-yellow-300 text-xs" data-color="#facc15" data-alpha="0.7" data-mode="tint">Kuning</button>
                </div>
                `;
    ...
    <script>
        ...
        document.querySelector('a[href="{{ route('orderHoodie') }}"]').addEventListener('click', function(event) {
            saveMockupImage();
            simpanHargaSablon();
            event.preventDefault();
            window.location.href = "{{ route('orderHoodie') }}";
        });
    </script>
@endpush
